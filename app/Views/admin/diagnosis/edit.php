...
<?php $this->section('content'); ?>
        ...
        <div class="mb-6">
            <div class="flex justify-between items-center">
                <div>
                    ...
                    <p class="text-gray-600 mt-1">Update diagnosis for this device</p>
                </div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-4 gap-6">
            ...
            <div class="lg:col-span-1 space-y-6">
                    ...
                    <div class="space-y-3 text-sm">
                        ...
                        <div>
                            ...
                            <span class="ml-2 px-2 py-1 rounded-full text-xs font-medium
                              <?= ($order['diagnosis_status'] === 'completed') ? 'bg-green-100 text-green-800' : 'bg-yellow-100 text-yellow-800' ?>">
                            <?= ucfirst(str_replace('_', ' ', $order['diagnosis_status'])) ?>
                        </span>
                        </div>
                    </div>
                </div>

                <!-- Original Diagnosis Info -->
                <div class="bg-blue-50 border border-blue-200 rounded-xl p-6">
                    <h3 class="text-lg font-semibold text-blue-800 mb-4">Original Diagnosis</h3>
                    <div class="space-y-3 text-sm">
                        <?php if ($order['diagnosed_by_name']): ?>
                            <div>
                                <span class="text-blue-700">Diagnosed by:</span>
                                <span class="font-medium ml-2"><?= $order['diagnosed_by_name'] ?></span>
                            </div>
                        <?php endif; ?>
                        <?php if ($order['diagnosis_date']): ?>
                            <div>
                                <span class="text-blue-700">Date:</span>
                                <span class="font-medium ml-2"><?= date('M d, Y H:i', strtotime($order['diagnosis_date'])) ?></span>
                            </div>
                        <?php endif; ?>
                    </div>
                </div>
                ...
                <?php if (!empty($templates)): ?>
                    <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
                        <h3 class="text-lg font-semibold text-gray-800 mb-4">Templates</h3>
                        <div class="space-y-2">
            ...
            <div class="lg:col-span-3">
                <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
                    ...
                    <form action="/admin/diagnosis/<?= $order['id'] ?>" method="POST" class="space-y-8" id="diagnosisForm">
                        <?= csrf_field() ?>
                        ...
                        <div>
                            ...
                            <div id="issuesContainer">
                                <?php
                                $issues = [];
                                if (!empty($order['issues_found'])) {
                                    $issues = is_string($order['issues_found']) ? json_decode($order['issues_found'], true) : $order['issues_found'];
                                }

                                if (empty($issues)) {
                                    $issues = [['description' => '', 'severity' => 'medium', 'repair_needed' => true]];
                                }

                                foreach ($issues as $index => $issue): ?>
                                    <div class="issue-item bg-gray-50 p-4 rounded-lg mb-4" data-issue-index="<?= $index ?>">
                                        <div class="flex justify-between items-center mb-3">
                                            <h4 class="font-medium text-gray-800">Issue #<?= $index + 1 ?></h4>
                                            <button type="button" onclick="removeIssueField(this)"
                                                    class="text-red-600 hover:text-red-800 text-sm <?= count($issues) <= 1 ? 'hidden' : '' ?>">
                                        ...
                                        <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                                            <div class="md:col-span-2">
                                                ...
                                                <input type="text" name="issues[<?= $index ?>][description]" required
                                                       value="<?= htmlspecialchars($issue['description'] ?? '') ?>"
                                                       class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                                                       placeholder="Describe the issue found...">
                                            </div>
                                            <div>
                                                <label class="block text-sm font-medium text-gray-700 mb-2">Severity</label>
                                                <select name="issues[<?= $index ?>][severity]"
                                                        class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent">
                                                    <option value="low" <?= ($issue['severity'] ?? 'medium') === 'low' ? 'selected' : '' ?>>Low</option>
                                                    <option value="medium" <?= ($issue['severity'] ?? 'medium') === 'medium' ? 'selected' : '' ?>>Medium</option>
                                                    <option value="high" <?= ($issue['severity'] ?? 'medium') === 'high' ? 'selected' : '' ?>>High</option>
                                                </select>
                                            </div>
                                        </div>
                                        ...
                                        <div class="mt-3">
                                            <label class="flex items-center">
                                                <input type="hidden" name="issues[<?= $index ?>][repair_needed]" value="0">
                                                <input type="checkbox" name="issues[<?= $index ?>][repair_needed]" value="1"
                                                    <?= ($issue['repair_needed'] ?? true) ? 'checked' : '' ?>
                        ...
                        <div>
                            ...
                            <textarea name="diagnosis_notes" id="diagnosis_notes" rows="6" required
                                      class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                                      placeholder="Provide detailed diagnosis notes including your findings, inspection results, and technical assessment..."><?= htmlspecialchars(old('diagnosis_notes', $order['diagnosis_notes'] ?? '')) ?></textarea>
                        ...
                        <div>
                            ...
                            <textarea name="recommended_actions" id="recommended_actions" rows="5" required
                                      class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                                      placeholder="Describe the recommended repair actions, parts needed, and any precautions..."><?= htmlspecialchars(old('recommended_actions', $order['recommended_actions'] ?? '')) ?></textarea>
                            <p class="text-xs text-gray-500 mt-1">Minimum 10 characters required</p>
                        </div>
                        ...
                        <div>
                            ...
                            <div class="flex items-center space-x-4">
                                <input type="number" name="estimated_hours" id="estimated_hours" step="0.5" min="0" max="100"
                                       value="<?= old('estimated_hours', $order['estimated_hours'] ?? '') ?>"
                        ...
                        <div class="border-t border-gray-200 pt-6">
                            <h3 class="text-lg font-semibold text-gray-800 mb-4">Edit Information</h3>
                                    ...
                                    <div class="ml-3">
                                        ...
                                        <div class="mt-2 text-sm text-yellow-700">
                                            <ul class="list-disc pl-5 space-y-1">
                                                <li>Changes will be logged in the order history</li>
                                                <li>Customer will be notified of significant changes</li>
                                                <li>If quotation exists, it may need to be updated</li>
                                            </ul>
                                        </div>
                                    </div>
                                </div>
                            ...
                            <div class="mt-4">
                                ...
                                <textarea name="edit_reason" id="edit_reason" rows="3"
                                          class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                                          placeholder="Briefly explain why this diagnosis is being updated..."><?= old('edit_reason') ?></textarea>
                            </div>
                        </div>
                        ...
                        <div class="flex justify-end space-x-4 pt-6 border-t border-gray-200">
                            ...
                            <button type="submit"
                                ...
                                <i class="fas fa-check mr-2"></i>Update Diagnosis
                            </button>
                        </div>
                    </form>
    ...
    <script>
        ...
        function saveDraft() {
            ...
            localStorage.setItem(`diagnosis_edit_draft_${<?= $order['id'] ?>}`, JSON.stringify(Object.fromEntries(formData)));
